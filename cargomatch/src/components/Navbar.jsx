...
export default function Navbar() {
  ...
  // Fetch unread notification + message counts, update in real time
  useEffect(() => {
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
            ...
            {user ? (
              <>
                <Link to="/notifications" className={`relative p-2 rounded-lg transition-colors ${isActive('/notifications') ? 'bg-forest-50 text-forest-600' : 'text-stone-500 hover:bg-stone-100'}`}>
                  <Bell size={18} />
                  {unreadNotifs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-rose-500 rounded-full border border-white" />}
                </Link>
                <Link to="/messages" className={`relative p-2 rounded-lg transition-colors ${isActive('/messages') ? 'bg-forest-50 text-forest-600' : 'text-stone-500 hover:bg-stone-100'}`}>
                  ...
                  {unreadMsgs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-forest-500 rounded-full border border-white" />}
                </Link>
          ...
          <div className="md:hidden flex items-center gap-1">
            {user && (
              <>
                <Link to="/notifications" className="relative p-2 text-stone-600"><Bell size={18} />{unreadNotifs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-rose-500 rounded-full border border-white" />}</Link>
                <Link to="/messages" className="relative p-2 text-stone-600"><MessageSquare size={18} />{unreadMsgs > 0 && <span className="absolute top-1 right-1 w-2 h-2 bg-forest-500 rounded-full border border-white" />}</Link>
              </>
            )}
