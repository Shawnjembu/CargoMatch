  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('carrier-realtime')
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'loads' }, (payload) => {
        // Remove loads that are no longer pending (matched, cancelled, etc.)
        if (payload.new.status !== 'pending') {
          setAvailableLoads(prev => prev.filter(l => l.id !== payload.new.id))
        }
      })
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'load_bids' }, (payload) => {
        // When a bid is accepted or rejected, refresh data to update UI
        if (payload.new.status !== 'pending') {
          fetchData()
        }
      })
