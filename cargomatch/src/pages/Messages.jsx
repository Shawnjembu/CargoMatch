...
import { Send, Search, Package, MapPin, CheckCheck, MessageSquare, Navigation, ArrowLeft, Headphones } from 'lucide-react'
...
export default function Messages() {
  ...
  const [supportConvo,    setSupportConvo]     = useState(null)   // support thread convo object
  ...
  useEffect(() => {
    if (!activeId || activeId === SUPPORT_ID) return
    if (convos.length > 0) {
      const c = convos.find(c => c.id === activeId)
      if (c && c.unread > 0) markRead(activeId)
    }
  }, [activeId, convos])
  ...
  }, [activeId, convos, supportConvo])

  const fetchSupportThread = async () => {
    if (!user) return
    // For admins: don't show a personal support thread
    ...
    setSupportConvo({
      id:          SUPPORT_ID,
      threadId:    thread.id,
      name:        'CargoMatch Support',
      avatar:      'S',
      avatarColor: 'bg-violet-100 text-violet-700',
      lastMessage: last?.body || 'Send us a message',
    ...
    // Mark support messages read if this thread is active
    if (activeIdRef.current === SUPPORT_ID && unread > 0) {
      await supabase.from('support_messages')
        .update({ read: true })
        .eq('thread_id', thread.id)
        .neq('sender_id', user.id)
      setSupportConvo(prev => prev ? { ...prev, unread: 0, messages: prev.messages.map(m => ({ ...m, read: true })) } : prev)
    }
  }
    ...
    try {
      ...
      if (shipments?.length) {
        const enriched = await Promise.all(shipments.map(async s => {
          ...
          const otherName = isShipper ? (s.carrier?.company_name || 'Carrier') : (s.shipper?.full_name || 'Shipper')
          const otherId   = isShipper ? s.carrier?.user_id : s.shipper?.id

          const { data: msgs } = await supabase
            .from('messages').select('*').eq('shipment_id', s.id).order('created_at', { ascending: true })
      ...
      // Always fetch support thread too
      fetchSupportThread()
    } catch (err) {
  ...
  const active    = isSupport ? supportConvo : (convos.find(c => c.id === activeId) || convos[0])
  ...
  const sendMessage = async () => {
    ...
    if (isSupport) {
      if (!supportConvo?.threadId) { setSending(false); return }
      const { error } = await supabase.from('support_messages').insert({
        thread_id: supportConvo.threadId,
        sender_id: user.id,
        body,
      })
      if (error) {
  ...
  const markRead = async (cId) => {
    if (cId === SUPPORT_ID) {
      if (!supportConvo?.threadId) return
      await supabase.from('support_messages')
        .update({ read: true })
        .eq('thread_id', supportConvo.threadId)
        .neq('sender_id', user.id)
      setSupportConvo(prev => prev ? { ...prev, unread: 0, messages: prev.messages.map(m => ({ ...m, read: true })) } : prev)
      return
    }
  ...
  const ConvoItem = ({ c, active: isActive }) => (
    <button
      onClick={() => handleSelectConvo(c.id)}
      className={`w-full text-left px-4 py-3.5 border-b border-stone-50 transition-colors hover:bg-stone-50 ${isActive ? 'bg-forest-50 border-l-2 border-l-forest-500' : ''}`}>
      <div className="flex items-start gap-3">
        <div className="relative flex-shrink-0">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-display font-700 text-sm ${c.avatarColor}`}>
            {c._isSupport ? <Headphones size={16} /> : c.avatar}
          </div>
        </div>
      ...
      <div className="flex-1 flex pt-16 overflow-hidden" style={{ maxHeight: '100vh' }}>
        ...
        <div className={`${activeId ? 'hidden md:flex' : 'flex'} w-full md:w-80 flex-shrink-0 border-r border-stone-200 bg-white flex-col`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {/* Support thread — pinned at top, always visible */}
            {!profile?.is_admin && supportConvo && (
              <ConvoItem c={supportConvo} active={activeId === SUPPORT_ID} />
            )}
            ...
            {!loading && !fetchErr && filteredConvos.length === 0 && (
              <div className="p-6 text-center">
                <MessageSquare size={28} className="text-stone-300 mx-auto mb-2" />
                <p className="text-xs text-stone-400">No shipment conversations yet.</p>
        ...
        {active ? (
          <div className="flex-1 flex flex-col min-w-0 bg-stone-50 w-full">
            ...
            <div className="bg-white border-b border-stone-200 flex-shrink-0">
              <div className="px-4 md:px-6 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center font-display font-700 text-sm ${active.avatarColor}`}>
                    {active._isSupport ? <Headphones size={15} /> : active.avatar}
                  </div>
                  <div>
                    <span className="font-display font-700 text-stone-900 text-sm">{active.name}</span>
                    {active._isSupport ? (
                      <p className="text-xs text-stone-400 mt-0.5">Typically replies within a few hours</p>
                    ) : (
                ...
                {!active._isSupport && active.shipment_ref && (
              ...
              {/* Load context banner — shipment convos only */}
              {!active._isSupport && (active.cargo || active.price) && (
            ...
            <div className="flex-1 overflow-y-auto px-3 md:px-6 py-4 md:py-6 space-y-3">
              {active.messages.length === 0 && (
                <div className="text-center text-stone-400 text-sm py-12">
                  {active._isSupport ? 'How can we help? Send us a message.' : 'No messages yet. Say hello!'}
                </div>
              )}
                ...
                return (
                  <div key={msg.id}>
                    ...
                    <div className={`flex ${msg.from_me ? 'justify-end' : 'justify-start'}`}>
                      {!msg.from_me && (
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center font-display font-700 text-xs mr-2 flex-shrink-0 mt-1 ${active.avatarColor}`}>
                          {active._isSupport ? <Headphones size={11} /> : active.avatar}
                        </div>
                      )}
            ...
            <div className="bg-white border-t border-stone-200 px-4 pt-3 pb-3 flex-shrink-0">
              ...
              <div className="flex items-center gap-3">
                <input type="text" placeholder={active._isSupport ? 'Message support...' : 'Type a message...'}
                  value={input}
