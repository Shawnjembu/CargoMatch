...
export default function ShipperDashboard() {
  ...
  const [bidsModal,    setBidsModal]    = useState(null)  // { load, bids[], error? }
  ...
  const [acceptErr,    setAcceptErr]    = useState('')    // error message for accept flow
  ...
  useEffect(() => {
    ...
    // Realtime: notifications, messages, shipments, and bids
    const channel = supabase.channel('shipper-realtime')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shipments', filter: `shipper_id=eq.${user.id}` },
        fetchData)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'load_bids' },
        async (payload) => {
          // Refresh bid counts when bids are added, updated, or removed
          // This ensures the bid modal shows up-to-date information
  ...
  const openBidsModal = async (load) => {
    ...
    if (error) { setBidsModal({ load, bids: [], error: error.message }); return }
  ...
  const initPayment = async (load, bid) => {
    ...
    try {
      ...
      const [{ error: e1 }, , { error: e3 }] = await Promise.all([
        supabase.from('load_bids').update({ status: 'accepted' }).eq('id', bid.id),
        supabase.from('load_bids').update({ status: 'rejected' }).eq('load_id', load.id).neq('id', bid.id),
        supabase.from('loads').update({ status: 'matched' }).eq('id', load.id),
      ])
      if (e1) throw new Error('Failed to update bid: ' + e1.message)
      if (e3) throw new Error('Failed to update load: ' + e3.message)
      ...
      if (import.meta.env.DEV) console.error('[initPayment]', err)
    } finally {
  ...
  return (
    ...
    {bidsModal && (
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={() => setBidsModal(null)}>
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          {bidsModal.error && (
            <div className="mb-4 px-3 py-2.5 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700">
              Could not load bids: {bidsModal.error}
            </div>
          )}
          {bidsModal.bids.length === 0 && !bidsModal.error ? (
    ...
    {payModal && (
      <div className="fixed inset-0 z-[60] flex items-center justify-center px-4" onClick={() => { setPayModal(null); setAcceptErr('') }}>
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
          ...
          {acceptErr && (
            <div className="mb-4 px-3 py-2.5 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700">{acceptErr}</div>
          )}
          <div className="flex gap-3">
            <button onClick={() => { setPayModal(null); setAcceptErr('') }}
            ...
            <button
              onClick={() => initPayment(payModal.load, payModal.bid)}
              disabled={!!acceptingBid}
              className="flex-1 py-3 bg-forest-500 hover:bg-forest-600 disabled:bg-stone-200 disabled:text-stone-400 text-white text-sm font-medium rounded-xl transition-all flex items-center justify-center gap-2">
              <CreditCard size={14} /> {acceptingBid ? 'Processing...' : 'Confirm Carrier'}
            </button>
          </div>
        </div>
      </div>
